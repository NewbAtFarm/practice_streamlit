...
st.markdown('**포켓몬**을 하나씩 추가해서 도감에 추가하세요')
...
print('page reload, auto complete', auto_complete)
with st.form(key='form'):
    col1, col2 = st.columns(2)
    with col1:
        name = st.text_input(
            label='포켓몬 이름',
            value=example_pokemon['name'] if auto_complete == True else ''
        )
    ...
    with col2:
        types = st.multiselect(label='포켓몬 속성', 
                            ...
                            max_selections=2,
                            default=example_pokemon['types'] if auto_complete == True else None
                )
    image_url = st.text_input(
                            label='포켓몬 이미지 URL',
                            value=example_pokemon['image_url'] if auto_complete == True else ''
                            )
...
# 꼬부기 image https://i.namu.wiki/i/ff5cG_bYUqA83g1DPhvn5hM5bo3XbcCTHUzbQgp-f9my6ovQUXApyHbg1iuIz8TdIUKoeNBC-e8wVv9MwDPVP7gcqEZpWnSMapSNLYYmodoUXMVc7oY-td3ud1h7zqTclgFAcbEStw-93Gcy-ix4qg.webp
...
for i in range(0, len(st.session_state.pokemons), 3):
    ...
    for j in range(len(row_pokemons)):
        with cols[j]:
            pokemon = st.session_state.pokemons[i+j]
            with st.expander(label=f"**{str(i+j+1)}\. {pokemon['name']}**", expanded=True):
                st.image(pokemon['image_url'])
                emoji_types = [f'{type_emoji_dict[x]} {x}' for x in pokemon['types']]
                st.subheader(' / '.join(emoji_types))
                delete_button = st.button(label='삭제', key=i+j, use_container_width=True)
                if delete_button:
                    print(str(i+j), 'delete button clicked')
